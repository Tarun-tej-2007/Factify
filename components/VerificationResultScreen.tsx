...
export default function VerificationResultScreen() {
  ...
  if (!result) {
    // Nothing to show yet — caller should navigate here after running verification.
    return null;
  }
  ...
  const handleOpenLink = async () => {
    if (result.type === "link") {
      try {
        await WebBrowser.openBrowserAsync(input);
      } catch (error) {
          ...
          {result.type === "text" && (
            <>
              ...
              <View style={styles.textContainer}>
                <Text style={styles.contentText}>{input}</Text>
              </View>
            </>
          )}
        </Animated.View>
